...
import { JUEGOS, juegoAleatorio, type TipoJuego } from "../data/juegosAssets";
import { getCategoriasPublic, type CategoriaAPI } from "../lib/api";
import { buildJuegoRapidoState } from "../lib/gameSession";
...
export default function Inicio() {
  ...
  const juegoRapido = async () => {
    playClick();
    if (categorias.length === 0) return;
    const state = await buildJuegoRapidoState(categorias);
    if (!state) return;
    const juego = juegoAleatorio();
    navigate(`/juegos/${juego.tipo}`, { state });
  };
  ...
  return (
    <PageLayout mainClassName="pt-2 sm:pt-4">
      <div className="text-center mb-8 sm:mb-10">
        ...
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-[var(--text-dark)] mb-3 leading-tight px-2">
          ¡Aprende lengua de señas mientras juegas!
        </h1>
        <p className="text-lg sm:text-xl text-[var(--text-muted)] max-w-md mx-auto px-2">
      ...
      <div className="flex items-center gap-3 mb-6 max-w-xs mx-auto" aria-hidden="true">
        ...
        <span className="text-sm sm:text-base font-bold text-[var(--text-muted)] uppercase tracking-widest">
          Elige un juego
        </span>
        <div className="flex-1 h-px bg-[var(--beige-border)]" />
      </div>
      ...
      <div className="text-center">
        <Link
          to="/diccionario"
          className="touch-target-inline inline-flex items-center gap-2 px-6 sm:px-7 py-3 sm:py-4 rounded-full border-2 border-[var(--beige-border)] bg-white hover:border-[var(--green-primary)] hover:shadow-md transition-all no-underline text-[var(--text-dark)] font-bold text-base sm:text-lg group"
        >

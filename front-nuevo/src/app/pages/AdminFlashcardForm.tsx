...
export default function AdminFlashcardForm() {
  ...
  const nivelesFiltrados = useMemo(() => {
    ...
    return todosNiveles.filter((n: any) => {
      const id = n.categoria_id?._id || n.categoria_id;
      return id?.toString() === categoriaId;
    });
  }, [categoriaId, todosNiveles]);

  const handleCategoriaChange = (nuevaCategoriaId: string) => {
    setCategoriaId(nuevaCategoriaId);
    const niveles = todosNiveles.filter((n: any) => {
      const id = n.categoria_id?._id || n.categoria_id;
      return id?.toString() === nuevaCategoriaId;
    });
    setNivelId(niveles[0]?._id?.toString() || "");
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!categoriaId || !nivelId) {
      return setError("Selecciona categoría y nivel.");
    }
    if (!videoUrl?.trim()) {
        ...
        />
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-[20px] sm:rounded-[28px] border-2 border-[var(--beige-border)] p-4 sm:p-6 lg:p-8 shadow-lg space-y-5 sm:space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="categoria" className="font-semibold text-[var(--text-dark)]">
              ...
              <div className="space-y-2">
                <Label htmlFor="nivel" className="font-semibold text-[var(--text-dark)]">
                ...
                <select
                  ...
                  required
                  disabled={!categoriaId || nivelesFiltrados.length === 0}
                  className="w-full rounded-[16px] border-2 border-[var(--beige-border)] px-4 py-3 bg-white text-[var(--text-dark)] disabled:opacity-50"
                >
                  <option value="">
                    {!categoriaId
                      ? "Primero elige una categoría"
                      : nivelesFiltrados.length === 0
                      ? "Sin niveles — créalos en Niveles"
                      : "Seleccionar nivel"}
                  </option>
            ...
            {categoriaId && nivelesFiltrados.length === 0 && (
              <p className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-[12px] px-4 py-2 m-0">
                Esta categoría no tiene niveles. Ve a <strong>Niveles</strong> en el menú para crear uno.
              </p>
            )}

...
export default function JuegoQuiz() {
  ...
  const { categoriaId, nivelId, cantidad } = location.state || {
  ...
  useEffect(() => {
    ...
    const loadWords = async () => {
      try {
        const filtros = { categoriaId, nivelId, limit: 50 };
        const res = await getPalabrasPublic(filtros);
        const words = shuffle(res.data.slice(0, Math.min(cantidad, res.data.length)));
        setPalabras(words);

        const allRes = await getPalabrasPublic({ ...filtros, limit: 200 });
              ...
              <div className="space-y-3 mb-6">
                {options.map((option, index) => (
                  <motion.button
                    ...
                    className={`w-full p-6 rounded-[28px] border-2 transition-all text-left font-bold text-xl sm:text-2xl ${

    ...
    >
      <div className="max-w-6xl mx-auto w-full grid grid-cols-[1fr_auto] md:grid-cols-[1fr_auto_1fr] items-center gap-x-3 sm:gap-x-4 min-h-[var(--touch-min)]">
          ...
          {NAV_PRINCIPAL.map(({ href, label, icon }) => {
            ...
            return (
              <Link
                ...
                className={`inline-flex items-center justify-center gap-2 h-11 px-4 rounded-2xl font-bold text-base no-underline transition-colors ${
                  active
        ...
        <div className="flex items-center justify-end gap-2 sm:gap-3 self-center h-full min-w-0 col-start-2 md:col-start-3">
          ...
          <Link
            to="/admin-login"
            className="inline-flex items-center justify-center gap-2 h-11 px-3 sm:px-4 rounded-2xl border-2 border-[var(--beige-border)] hover:border-[var(--green-primary)] hover:bg-white transition-all no-underline text-[var(--text-dark)] text-base font-bold shrink-0"
